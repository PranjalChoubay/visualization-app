...
export default function App() {
  ...
  // Effect to fetch and process chat data on component mount.
  useEffect(() => {
    // --- IMPORTANT ---
    // For this fetch to work, the 'whatsapp_chat_rohan_full.json' file
    // MUST be placed in the 'public' folder of your React project.
    fetch("/whatsapp_chat_rohan_full.json")
      .then((res) => {
        if (!res.ok) {
          throw new Error(`HTTP error! Status: ${res.status}. Make sure the JSON file is in the 'public' folder.`);
        }
        return res.json();
      })
      .then((data) => {
  ...
  const weeks = useMemo(() => {
    const weekOrder = (weekStr) => parseInt(weekStr.replace('Week ', ''), 10);
    if (selectedMonth === "All Months") {
      const allWeeks = [...new Set(messages.map((m) => m.week))];
      return ["All Weeks", ...allWeeks.sort((a, b) => weekOrder(a) - weekOrder(b))];
    }
    const weeksInMonth = messages
      .filter((m) => m.month === selectedMonth)
      .map((m) => m.week);
    const uniqueWeeks = [...new Set(weeksInMonth)];
    return ["All Weeks", ...uniqueWeeks.sort((a, b) => weekOrder(a) - weekOrder(b))];
  ...
  if (error) {
    return (
       <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center" minHeight="100vh" textAlign="center" p={2}>
